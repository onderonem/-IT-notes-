...
-- If we want to uptade more than one parameter, we use "IN" keyword                
...
-- MySQL does not work without BINARY keyvord for UPPER case rule!!!


INSERT INTO students08 VALUES (101, 'Ali Can', 23);
-- It cannot be inserted because of Uppercase rule

INSERT INTO students08 VALUES (102, 'ALIYE CAN', 20);
-- it cannot be inserted because of age restriction
...
SELECT * FROM clients WHERE product_name = 'Orange'
...
-- IN Keyword                        
-- We can make shorter above queri using by IN keyword
SELECT * FROM clients WHERE product_name IN ('Orange', 'Apple', 'Apricot');

-- NOT IN Keyword
-- We can also use the opposite keyword of in : NOT IN
...
-- Select all records where the product_id is not between 20 and 40 
-- => just product_id 10 
